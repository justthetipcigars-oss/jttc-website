...
import HumidorClient from './HumidorClient';
import { fetchAllProducts } from '@/lib/lightspeed';

export const revalidate = 3600;

export const metadata = { title: 'My Humidor | Just The Tip Cigars' };
...
export default async function HumidorPage() {
  ...
  const [{ data: items }, products] = await Promise.all([
    supabase.from('humidor').select('*').eq('user_id', user.id).order('added_at', { ascending: false }),
    ...
    <>
      ...
      <main style={{ background: 'var(--color-pitch)', minHeight: '100vh', paddingTop: '80px' }}>
        <div className="max-w-5xl mx-auto px-6 py-16">
          ...
          <HumidorClient initialItems={items ?? []} products={products} />
        </div>
      </main>
      <Footer />
    </>
