      ...
      {[1, 2, 3, 4, 5].map(n => (
        <button
          ...
          style={{
            ...
            fontSize: '1.85rem',
            color: n <= display ? 'var(--color-terracotta)' : 'rgba(255,255,255,0.18)',
            padding: '0 2px', lineHeight: 1, transition: 'color 0.1s',
          }}
        >★</button>
      ))}
    </div>
...
function FlavorMeter({ label, value, onChange }: { label: string; value: number | null; onChange: (v: number) => void }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '0.5rem' }}>
      <div style={{ width: '110px', flexShrink: 0, color: 'var(--color-cream-dark)', fontSize: '0.72rem', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
        {label}
      </div>
          ...
          return (
            <button
              ...
              style={{
                ...
                border: '1px solid',
                borderColor: active ? 'var(--color-terracotta)' : 'rgba(255,255,255,0.18)',
      ...
      {value !== null && (
        <span style={{ color: 'var(--color-terracotta)', fontSize: '0.7rem', letterSpacing: '0.05em' }}>
          {METER_LABELS[(value ?? 1) - 1]}
        </span>
      )}
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4" style={{ gap: '1.25rem', marginBottom: '1.25rem' }}>
          {([
        ...
        {/* Would You Try Again inline with ratings */}
        <div>
          <label style={labelStyle}>Would You Try Again?</label>
      ...
      {/* ── Date Smoked ─────────────────────────────────────── */}
      <section>
        <div style={sectionHead}>Date Smoked</div>
        <input
          type="date"
          value={form.date_smoked ?? ''}
          onChange={e => set('date_smoked', e.target.value)}
      ...
      <section>
        <div style={{ ...sectionHead, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <a
            href="/images/tasting_wheel_jttc.png"
            target="_blank"
            rel="noopener noreferrer"
            title="View Flavor Wheel"
            style={{ display: 'block', lineHeight: 0, borderRadius: '6px', overflow: 'hidden', border: '1px solid var(--color-charcoal-mid)' }}
          >
            <img
              src="/images/tasting_wheel_thumb.png"
              alt="JTTC Flavor Wheel"
              style={{ width: '54px', height: '54px', display: 'block', objectFit: 'cover' }}
            />
          </a>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {FLAVOR_CATEGORIES.map(cat => (
            <div key={cat.label}>
              <div style={{ color: 'var(--color-smoke)', fontSize: '0.65rem', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '0.4rem' }}>
                {cat.label}
              </div>
      ...
      <section>
        <button
          ...
          style={{
            ...
            fontFamily: 'inherit', padding: 0, marginBottom: showDetails ? '1rem' : 0,
          }}
        >
          <span>Cigar Details</span>
          <span style={{ fontSize: '0.75rem', opacity: 0.6 }}>{showDetails ? '▲ hide' : '▼ show'}</span>
        </button>
      ...
      <section>
        ...
        {form.band_photo_url && (
          <div style={{ maxWidth: '240px', border: '1px solid var(--color-charcoal-mid)', overflow: 'hidden', marginBottom: '0.75rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
          <button
            ...
            style={{
              ...
              border: '1px solid var(--color-charcoal-mid)', color: 'var(--color-cream)',
              fontSize: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase',
              cursor: photoUploading ? 'wait' : 'pointer', fontFamily: 'inherit',
            }}
          >
        ...
        {photoErr && <p style={{ color: '#e74c3c', fontSize: '0.75rem', margin: '0.5rem 0 0' }}>{photoErr}</p>}
      </section>

      {/* ── Save ────────────────────────────────────────────── */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap', paddingTop: '1rem', borderTop: '1px solid var(--color-charcoal-mid)' }}>
        <button
          type="button"
          onClick={handleSave}
          disabled={saving}
          style={{
            ...
            color: 'var(--color-cream)', border: 'none', fontFamily: 'inherit',
            fontWeight: 700, fontSize: '0.8rem', letterSpacing: '0.15em',
            textTransform: 'uppercase', cursor: saving ? 'wait' : 'pointer', transition: 'background 0.2s',
          }}
        >
            ...
            <div style={{ marginTop: '1rem', background: 'var(--color-charcoal)', border: '1px solid var(--color-charcoal-mid)' }}>
              {revisions.map((rev, i) => (
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      background: 'none', border: '1px solid var(--color-charcoal-mid)',
                      color: 'var(--color-smoke)', fontSize: '0.7rem', letterSpacing: '0.1em',
                      textTransform: 'uppercase', cursor: 'pointer', padding: '0.3rem 0.75rem', fontFamily: 'inherit',
                    }}
                  >
              ...
              {viewingRevision && (
                <div style={{ padding: '1.25rem', borderTop: '1px solid var(--color-charcoal-mid)', background: 'var(--color-pitch)' }}>
                  ...
                  <div className="grid grid-cols-2" style={{ gap: '0.5rem 1.5rem', fontSize: '0.82rem' }}>
                    {(['cigar_name', 'brand', 'size', 'wrapper', 'binder', 'filler', 'date_smoked', 'notes', 'body', 'flavor_intensity', 'strength', 'overall_rating', 'would_try_again'] as const).map(k => {
                      ...
                      return (
                        <div key={k} style={{ gridColumn: (k === 'notes' || k === 'cigar_name') ? '1 / -1' : 'auto' }}>
                          <span style={{ color: 'var(--color-smoke)', fontSize: '0.68rem', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                            {k.replace(/_/g, ' ')}:{' '}
                          </span>
                          <span style={{ color: 'var(--color-cream-dark)' }}>{String(val)}</span>
                        </div>
                      );
                    })}
